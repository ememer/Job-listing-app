...
const JobListProvider = ({ children }: Props) => {
    ...
    const displayFilteredJobs = () =>{
       let filteredData = jobLists.filter((jobItem)=> filtersArray.find((filter)=> jobItem.tools.includes(filter) || jobItem.languages.includes(filter)))
       return filteredData.length > 0 ? filteredData : jobLists
    }

  return (
    <JobListContext.Provider value={{jobLists, filtersArray, setFiltersArray, displayFilteredJobs}}>{children}</JobListContext.Provider>
  );
};
